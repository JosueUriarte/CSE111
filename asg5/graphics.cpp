...
int window::pixel_movement = 4;

int window::thickness = 4;
string window::color = "red";

bool window::select_bool = false;
bool window::text_bool = false;
...
void mouse::draw() {
   ...
   string myText;
   myText += "(" + to_string(xpos) + ","
                 + to_string(window::height - ypos) + ")";
   if (left_state == GLUT_DOWN) myText += "L"; 
   if (middle_state == GLUT_DOWN) myText += "M"; 
   if (right_state == GLUT_DOWN) myText += "R"; 
...
void window::display() {
   ...
   for (auto& object: window::objects){
      ...
      if(window::is_text()){
         window::set_text(false);
      }
      else{
         window::draw_num(object, idx);
      }
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         window::move_selected_object (-1*pixel_movement, 0);
         break;
      case 'J': case 'j':
         window::move_selected_object (0, -1*pixel_movement);
         break;
      case 'K': case 'k':
         window::move_selected_object (0, +1*pixel_movement);
         break;
      ...
      case 'P': case 'p': case BS:
         ...
         if (selected_obj == 0){
            selected_obj = window::objects.size();
         }
...
void window::special (int key, int x, int y) {
   ...
   switch (key) {
      case GLUT_KEY_LEFT:
         move_selected_object (-1*pixel_movement, 0); break;
      case GLUT_KEY_DOWN:
         move_selected_object (0, -1*pixel_movement); break;
      case GLUT_KEY_UP:
         move_selected_object (0, +1*pixel_movement); break;
      case GLUT_KEY_RIGHT:
         move_selected_object (+1*pixel_movement, 0); break;
...
void window::move_selected_object(float xdelta, float ydelta) {
   objects[selected_obj].move(xdelta,ydelta);
}
...
void window::draw_num(object obj, size_t idx) {
