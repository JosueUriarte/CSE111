...
void send_error(accepted_socket &client_sock,
                cix_header &header, string error_message)
{
   outlog << error_message << header.filename << endl;
...
void reply_put(accepted_socket& client_sock, cix_header& header){
   ...
   if (file.fail()){
      outlog << header.filename << ": ifstream failed: "
          << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet(client_sock, &header, sizeof header);
...
void reply_rm(accepted_socket &client_sock, cix_header &header){
   ...
   if(success < 0){
      outlog << header.filename << " ERROR: unlink failed "
          << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet(client_sock, &header, sizeof header);
   ...
   outlog << "Successfully removed " << header.filename << 
   " from server!" << endl;
